...
public class Seq<T> implements Iterable<T> {
	...
	public <K, V> ObjectMap<K, V> asMap(Function<T, K> keygen, Function<T, V> valgen) {
	...
	public <K> ObjectMap<K, T> asMap(Function<T, K> keygen) {
	...
	public <R> Seq<R> flatMap(Function<T, Iterable<R>> mapper) {
	...
	/** Returns a new array with the mapped values. */
	public <R> Seq<R> map(Function<T, R> mapper) {
		Seq<R> arr = new Seq<>(size);
		for (int i = 0; i < size; i++) {
	...
	public Seq<T> with(Consumer<Seq<T>> cons) {
		cons.accept(this);
		return this;
	}
	 ...
	 */
	public T pop(Supplier<T> constructor) {
	...
	public <U extends Comparable<? super U>> Seq<T> sortComparing(Function<? super T, ? extends U> keyExtractor) {
	 ...
	 * @see Seq#selectRanked(Comparator, int)
	 */
