 ...
 * @author Eipusino
 */
public class BoolList implements Cloneable {
	 ...
	 */
	public BoolList(boolean ordered, int capacity) {
	 ...
	 * grown.
	 */
	public BoolList(BoolList array) {
	 ...
	 *                memory copy.
	 */
	public BoolList(boolean ordered, boolean[] array, int startIndex, int count) {
		this(ordered, count);
		size = count;
		System.arraycopy(array, startIndex, items, 0, count);
	}

	/** @see #BoolList(boolean[]) */
	public static BoolList with(boolean... array) {
		return new BoolList(array);
	}

	public BoolList copy() {
		try {
			BoolList out = (BoolList) super.clone();
			out.items = new boolean[size];
	...
	public void addAll(BoolList array) {
		addAll(array.items, 0, array.size);
	}

	public void addAll(BoolList array, int offset, int length) {
	 ...
	 */
	public boolean removeAll(BoolList array) {
		int tmpSize = size;
		int startSize = tmpSize;
	...
	public void shuffle() {
		for (int i = size - 1; i >= 0; i--) {
			int ii = ThreadLocalRandom.current().nextInt(i);
			boolean temp = items[i];
			items[i] = items[ii];
			items[ii] = temp;
		}
	...
	public boolean random() {
		...
		return items[ThreadLocalRandom.current().nextInt(0, size - 1)];
	}
	...
	public boolean equals(Object object) {
		...
		if (!(object instanceof BoolList array)) return false;
		if (!array.ordered) return false;
		int n = size;
		if (n != array.size) return false;
		boolean[] items1 = items;
