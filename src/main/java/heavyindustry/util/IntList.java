...
public class IntList implements Cloneable {
	...
	public static IntList range(int min, int max) {
	...
	public IntList copy() {
		try {
			IntList out = (IntList) super.clone();
			out.items = new int[size];
	...
	public void chunked(int chunkSize, Consumer<int[]> iterator) {
		for (int i = 0; i < size; i += chunkSize) {
			int[] slice = Arrays.copyOfRange(items, i, Math.min(i + chunkSize, size));
			iterator.accept(slice);
		}
	}
	...
	public boolean equals(Object object) {
		...
		if (!(object instanceof IntList array)) return false;
