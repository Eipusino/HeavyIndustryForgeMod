...
public class IntSet implements Cloneable {
	...
	protected IntSetIterator iterator1, iterator2;
	...
	public IntSet copy() {
		try {
			IntSet out = (IntSet) super.clone();
			out.keyTable = new int[size];
	...
	protected void push(int insertKey, int index1, int key1, int index2, int key2, int index3, int key3) {
	...
	protected void addStash(int key) {
	...
	public class IntSetIterator {
		...
		protected void findNextIndex() {
			hasNext = false;
			int[] keyTable = IntSet.this.keyTable;
		...
		public IntList toList() {
			IntList list = new IntList(true, size);
			while (hasNext)
				list.add(next());
			return list;
		}
